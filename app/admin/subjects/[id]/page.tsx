...
async function createUnit(formData: FormData) {
  ...
  await prisma.unit.create({
    data: {
      name, // <-- ¡Solucionado! Ahora usa 'name' tal como espera tu base de datos
      subjectId,
            ...
            {subject.units.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {subject.units.map((unit) => (
                  // <-- ¡Solucionado! Mapeamos el 'name' de la BD al 'title' que espera el componente
                  <UnitRow key={unit.id} unit={{ id: unit.id, title: unit.name }} subjectId={subject.id} />
                ))}
              </div>
            )}
          </div>
